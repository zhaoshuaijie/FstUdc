...
public class VoiceManager {
     ...
     * 完成录音
     */
    public void stopVoiceRecord(boolean iscomplete) {
        try {
            mHandler.removeMessages(MSG_TIME_INTERVAL);
            mDeviceState = MEDIA_STATE_RECORD_STOP;
            stopRecorder(mMediaRecorder, true);
            mMediaRecorder = null;
            if (VoiceTimeUtils.timeSpanSecond(mRecTimeSum).mSpanSecond < 3) {
                if (iscomplete) {
                    Toast.makeText(mContext, "时间过短,请录制3s以上！", Toast.LENGTH_SHORT).show();
                }
            } else {
                ...
                if (file != null && file.length() > 0) {
                    ...
                    if (iscomplete) {
                        ...
                        if (voiceRecordCallBack != null) {
                            ...
                            voiceRecordCallBack.recFinish(0, null, null);
                        }
                    }
                }
            }
